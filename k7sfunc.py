...
def RIFE_NV(
	input : vs.VideoNode,
	lt_d2k : bool = False,
	model : typing.Literal[450, 451, 452] = 450,
	ext_proc : bool = True,
	t_tta : bool = False,
	fps_in : float = 23.976,
	fps_num : int = 2,
	...
	func_name = "RIFE_NV"
	if not isinstance(input, vs.VideoNode) :
		raise vs.Error(f"Module {func_name} invalid sub-parameter input")
	if not isinstance(lt_d2k, bool) :
		raise vs.Error(f"Module {func_name} invalid sub-parameter lt_d2k")
	if model not in [450, 451, 452] :
		raise vs.Error(f"Module {func_name} invalid sub-parameter model")
	if not isinstance(ext_proc, bool) :
		raise vs.Error(f"Module {func_name} invalid sub-parameter ext_proc")
	if not isinstance(t_tta, bool) :
		raise vs.Error(f"Module {func_name} invalid sub-parameter t_tta")
	if not isinstance(fps_in, (int, float)) or fps_in <= 0.0 :
		raise vs.Error(f"Module {func_name} invalid sub-parameter fps_in")
	if not isinstance(fps_num, int) or fps_num < 2 :
		raise vs.Error(f"Module {func_name} invalid sub-parameter fps_num")
	if not isinstance(fps_den, int) or fps_den >= fps_num or fps_num/fps_den <= 1 :
		raise vs.Error(f"Module {func_name} invalid sub-parameter fps_den")
	if sc_mode not in [0, 1, 2] :
		raise vs.Error(f"Module {func_name} invalid sub-parameter sc_mode")
	if gpu not in [0, 1, 2] :
		raise vs.Error(f"Module {func_name} invalid sub-parameter gpu")
	if not isinstance(gpu_t, int) or gpu_t <= 0 :
		raise vs.Error(f"Module {func_name} invalid sub-parameter gpu_t")
	if not isinstance(st_eng, bool) :
		raise vs.Error(f"Module {func_name} invalid sub-parameter st_eng")
	if not isinstance(ws_size, int) or ws_size < 0 :
		raise vs.Error(f"Module {func_name} invalid sub-parameter ws_size")
	if not isinstance(vs_t, int) or vs_t > vs_thd_init :
		raise vs.Error(f"Module {func_name} invalid sub-parameter vs_t")

	if not hasattr(core, "trt") :
		raise ModuleNotFoundError(f"Module {func_name} dependency error: missing plugin, check trt project")
	if sc_mode == 1 :
		if not hasattr(core, "misc") :
			raise ModuleNotFoundError(f"Module {func_name} dependency error: missing plugin, check misc project")
	elif sc_mode == 2 :
		if not hasattr(core, "mv") :
			raise ModuleNotFoundError(f"Module {func_name} dependency error: missing plugin, check mv project")
	if not (fps_num/fps_den).is_integer() :
		if not hasattr(core, "akarin") :
			raise ModuleNotFoundError(f"Module {func_name} dependency error: missing plugin, check akarin project")

	plg_dir = os.path.dirname(core.trt.Version()["path"]).decode()
	mdl_pname = "rife/" if ext_proc else "rife_v2/"
	if t_tta :
		mdl_fname = ["rife_v4.50", "rife_v4.51", "rife_v4.52"][[450, 451, 452].index(model)]
	else :
		mdl_fname = ["rife_v4.50", "rife_v4.51", "rife_v4.52"][[450, 451, 452].index(model)]
	mdl_pth = plg_dir + "/models/" + mdl_pname + mdl_fname + ".onnx"
	if not os.path.exists(mdl_pth) :
		raise vs.Error(f"Module {func_name} requested model is missing")
	...
	if vsmlrt is None :
		try :
			...
			raise ImportError(f"Module {func_name} dependency error: missing vsmlrt script")
	if LooseVersion(vsmlrt.__version__) < LooseVersion("3.20.4") :
		raise ImportError(f"Module {func_name} dependency error: vsmlrt version is too low, at least 3.20.4 required")
	...
	if (not lt_d2k and (size_in > 2048 * 1088)) or (size_in > 4096 * 2176) :
		raise Exception("Source resolution exceeds limit, temporarily halted.")
	if not st_eng and (((w_in > 4096) or (h_in > 2176)) or ((w_in < 289) or (h_in < 225))) :
		raise Exception("Source resolution not supported by dynamic engine, temporarily halted.")
